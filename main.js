...
// GLOBAL GAME STATE
// ----------------------------------------------------
let gameStarted = false; // <-- NEW: prevents flicker zap during start menu
...
function playFlickerZap() {
  if (!gameStarted) return; // <-- NEW: prevents zap before game
  flickerZap.pause();
  flickerZap.currentTime = 0;
  flickerZap.volume = 0.3;
  flickerZap.play();
}
...
continueBtn.addEventListener("click", () => {
  ...
  setTimeout(() => {
    ...
    // GAME HAS FULLY STARTED
    gameStarted = true; // <-- NEW LINE
...
document.getElementById("escape-btn").addEventListener("click", () => {
  document.getElementById("escape-popup").style.display = "none";
});

document.getElementById("scare-btn").addEventListener("click", () => {
  document.getElementById("scare-popup").style.display = "none";
});
...
function showScarePopup() {
  const scarePopUp = document.getElementById("scare-popup");
